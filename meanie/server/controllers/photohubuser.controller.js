...
router.use('/', express.static('../client/photohubuser')); // serve photohubuser front end files from '/photohubuser'
...
function ensureAuthenticated(req, res, next) {
    // use session auth to secure the front end photohubuser files
    if (!req.session.token) {
        return res.redirect('/login?returnUrl=' + encodeURIComponent('/photohubuser' + req.path));
    }
...
function getUpload() {
    ...
    var uploadDir = '../client/photohubhome/_content/uploads';
